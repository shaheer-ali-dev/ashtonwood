...
type PaymentPlan = 'upfront' | 'quarterly' | 'half' | null

const PLAN_LABELS: Record<Exclude<PaymentPlan, null>, string> = {
  upfront:   'Full Upfront — $3,000 USD',
  quarterly: 'Quarterly — $750 USD every 3 months',
  half:      'Half & Half — $1,500 USD now, $1,500 next month',
}

// ── TOS text verbatim from uploaded PDF ──────────────────────────────────────
...
export default function Agreement() {
  ...
  const handleSubmit = async () => {
    ...
    if (sigRef.current?.isEmpty()) newErrors.signature = 'Signature is required'
    ...
    try {
      ...
      ctx.fillText('SCULPT BY ASHTON', canvas.width / 2, y); y += 30
      ...
      const img = new Image(); img.src = signatureData
      await img.decode()
  ...
  const errSpan = (key: string) =>
    errors[key] ? <span style={{ color: '#c00', fontSize: 12, display: 'block', marginTop: 3, marginBottom: 8 }}>{errors[key]}</span> : null

  const planCard = (value: Exclude<PaymentPlan, null>, label: string, sub: string) => {
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            <div style={{
              border: `2px solid ${errors.signature ? '#c00' : '#ccc'}`,
              borderRadius: 10, background: '#fff', padding: 4,
            }}>
              <SignatureCanvas
                ref={sigRef}
                canvasProps={{ width: 390, height: 155, style: { background: '#fff', borderRadius: 8, display: 'block' } }}
                onBegin={() => setErrors((err) => { const n = { ...err }; delete n.signature; return n })}
              />
            </div>
            {errSpan('signature')}
            <button type="button" onClick={clearSignature}
              style={{ background: 'none', border: 'none', color: '#666', cursor: 'pointer', fontSize: 12.5, padding: '4px 0', marginTop: 2 }}>
              ↺ Clear Signature
            </button>
          </div>

          {/* Selected plan confirmation pill */}
          {plan && (
            <div style={{
